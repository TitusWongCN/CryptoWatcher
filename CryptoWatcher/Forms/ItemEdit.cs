        ...
        private async void saveBtn_Click(object sender, EventArgs e)
        {
            if (CheckValid())
            {
                try
                {
                    ...
                    string priceData = await WebAPIs.GetCurrentPrice(Result.CybermoneyName, Result.CurrencyName);
                    string[] datas = priceData.Split(',');
                    Result.Price = float.Parse(datas[0]);
                    Result.PercentChange = float.Parse(datas[1]);
