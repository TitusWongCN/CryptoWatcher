    ...
    public class Worker : IDisposable
    {
        ...
        public async void StartWork()
        {
            while (!Closed)
            {
                ...
                {
                    UpdateStatus("正在获取...");
                    string priceData = await WebAPIs.GetCurrentPrice(CryptoItem.CybermoneyName, CryptoItem.CurrencyName);
                    string[] datas = priceData.Split(',');
                    CryptoItem.Price = float.Parse(datas[0]);
                    CryptoItem.PercentChange = float.Parse(datas[1]);
        ...
        private void UpdateUI()
        {
            ...
            Binding.SubItems[2].Text = CryptoItem.PercentChange.ToString() + "%";
        }
